...
import { mapActions } from 'vuex' // mapActions函数用于生成methods中的函数
import localeMixin from '@/locales/mixin.js' // 语言变更用，数据未配置
export default {
  mixins: [
    localeMixin
  ],
  ...
  methods: {
    ...mapActions('d2admin/account', [ // 在组件中分发action hsm
      'login', 'code'
    ]),
    refreshTime () {
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          ...
          this.login({
            ...
            check_code: this.formLogin.check_code
          })
            .then(() => {
              // 重定向对象不存在则返回顶层路径
