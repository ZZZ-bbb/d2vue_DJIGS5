    ...
    <template slot="header">
      <div class="page-edition-upload" flex="main:justify cross:center">
        <div>
          ...
          <span>{{`共有 ${total} 条记录，当前显示第 ${Math.min((currentPage-1)*pageSize+1, editionData.length)} 至第 ${Math.min(currentPage*pageSize, editionData.length)} 条记录`}}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    downloadData (index) {
      ...
      const Edition_ID = this.editionData[currentIndex].Edition_ID
