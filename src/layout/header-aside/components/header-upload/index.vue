...
export default {
  ...
  methods: {
    ...
    uploadProgress (file, percentage) {
      file.statusText = file.getStatus()
      const done = Math.round((Number(file.offsetChunk) / file.totalChunk) * 100)
      const doing = Math.round(((file.totalChunk - Number(file.offsetChunk)) / file.totalChunk) * 100) / 100
      file.percentage = done + Math.floor(doing * Math.floor(percentage * 100))
      // console.log('<<<<<<<<<<<')
      // console.log(doing)
      // console.log(done + doing * Math.floor(percentage * 100))
    },
    /**
     * @description 上传成功
     */
