import low from 'lowdb' // 轻量级本地JSON数据库
import LocalStorage from 'lowdb/adapters/LocalStorage'
import util from '@/libs/util'
import { cloneDeep } from 'lodash'

const adapter = new LocalStorage(`d2admin-${process.env.VUE_APP_VERSION}`) // 申明一个适配器
const db = low(adapter)

// 实例化数据库，此处创建了sys和database两个数据库 hsm
db
  .defaults({
    sys: {},
    database: {}
 ...
 * @description user为true，则路径为{dbName}.user.{uuid}.{path}
 * @description user为false，则路径为{dbName}.public.{path}
